...
function editLyrics() {
  let search_id = CURRENT_SONG_ID;
  let song = copySong(search_id, "id");
  ...
  // if selected from set list, change lyrics in current node as well 
  // (as reload database does not change it)
  // select the current song as well
  let target_node = document.querySelector("#set_list_items > li[data-song-no='" + 
      String(CURRENT_SET_SONG_NO) + "']");

  if (target_node !== null) {
    target_node.setAttribute("data-lyrics", document
        .getElementById("edit_lyrics_input").value);
